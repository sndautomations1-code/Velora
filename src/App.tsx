  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isMobileMenuOpen ? 'bg-cream shadow-luxury' : isScrolled ? 'glass-effect shadow-luxury' : 'bg-transparent'
      }`}
    >
      <div className="container-padding max-w-7xl mx-auto">
        <div className="flex items-center justify-between h-20 md:h-24">
          ...
          <div className="hidden lg:flex items-center flex-1 justify-center gap-2 xl:gap-3 mx-6">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
          ...
          <button
            ...
            className={`lg:hidden p-2 transition-colors ${useDarkHeader ? 'text-charcoal' : 'text-white hero-text-shadow'}`}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-cream border-t border-gold/15"
          >
...
function DemoBanner() {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#1a1a1a] border-t border-gold/30 py-2.5 px-4">
      <p className="text-center text-xs sm:text-[13px] text-champagne leading-relaxed">
        <span className="text-gold-light mr-1.5" aria-hidden="true">&#10038;</span>
        This is a live demo by Splendessa — your clinic&apos;s site can look like this in 7 days
      </p>
    </div>
  );
}
